...
use iced_winit::winit::keyboard::NamedKey;
...
impl ApplicationHandler<UserEvent> for App {
    ...
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let state = program::State::new(controls, viewport.logical_size(), &mut renderer);
        ...
        match event {
            ...
            UserEvent::Back => {
                if let Some(app_data) = &mut self.app_data {
                    app_data.state.queue_message(uniquiz::Message::Back);
                    let _ = app_data.state.update(
                        app_data.viewport.logical_size(),
                        self.cursor_position
                            .map(|p| {
                                conversion::cursor_position(p, app_data.viewport.scale_factor())
                            })
                            .map(mouse::Cursor::Available)
                            .unwrap_or(mouse::Cursor::Unavailable),
                        &mut app_data.renderer,
                        &Theme::Ferra,
                        &renderer::Style {
                            text_color: Theme::Ferra.palette().text,
                        },
                        &mut app_data.clipboard,
                        &mut app_data.debug,
                    );
            ...
            UserEvent::Task(message) => {
                if let Some(app_data) = &mut self.app_data {
                    app_data.state.queue_message(message);
                    let _ = app_data.state.update(
                        app_data.viewport.logical_size(),
                        self.cursor_position
                            .map(|p| {
            ...
            WindowEvent::KeyboardInput {
                ...
                match event.logical_key {
                    winit::keyboard::Key::Named(NamedKey::GoBack) => {
                        self.proxy.send_event(UserEvent::Back);
                    }
                    _ => {}
                }
